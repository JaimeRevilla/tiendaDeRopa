					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Pantalon(p.getCodigo(), p.getColor(), p.getNombre(), p.getPrecio(), canti, p.getMarca(),p.getRutaFoto(), TipoPantalon.CHANDAL)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Pantalon(p.getCodigo(), p.getColor(), p.getNombre(), p.getPrecio(), canti, p.getMarca(),p.getRutaFoto(), TipoPantalon.VAQUEROS)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
									num = BD.contarProductos(con);
								} catch (SQLException e2) {
					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Pantalon(p.getCodigo(), p.getColor(), p.getNombre(), p.getPrecio(), canti, p.getMarca(),p.getRutaFoto(), TipoPantalon.CAMPANA)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
									num = BD.contarProductos(con);
								} catch (SQLException e2) {
