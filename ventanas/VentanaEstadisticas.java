...
//		contentPane.setImage(fondo);
		...
		lblMaxPrecioPedido.setText("PEDIDO M�S CARO DE TODAS LAS COMPRAS: " + max + " euros");
		panelCentralArriba.add(lblMaxPrecioPedido);
		
		lblMinPrecioPedido = new JLabel();
		double min = DineroDePedidoMasBarato();
		lblMinPrecioPedido.setText("PEDIDO M�S BARATO DE TODAS LA COMPRAS: " + min + " euros");
		...
		VentanaPrincipal.log.log(Level.INFO, "Estad�sticas generales obtenidas correctamente");
		...
		btnObtenerEstadisticas.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (index != -1) {
					...
					VentanaPrincipal.log.log(Level.INFO, "Estad�sticas de " + selec + " obtenidas correctamente");
					...
					if (VentanaPrincipal.tmPedidos.get(selec) == null || VentanaPrincipal.tmPedidos.get(selec).size() == 0) {
						...
						lblMaxCliente = "PEDIDO M�S CARO DE LA COMPRA DE " + selec + ": " + maxCliente + " euros";
					...
					if (VentanaPrincipal.tmPedidos.get(selec) == null || VentanaPrincipal.tmPedidos.get(selec).size() == 0) {
						...
						lblMinCliente = "PEDIDO M�S BARATO DE LA COMPRA DE " + selec + ": " + minCliente + " euros";
	...
	/**METODO PARA SABER CUANTOS PEDIDOS (PRODUCTOS DEL MISMO TIPO Y SU CANTIDAD) VA A COMPRAR EL USUARIO CUYO NOMBRE ES EL DADO POR P�RAMETRO
	...
	/**
	 * METODO PARA OBTENER EL PEDIDO DENTRO DE LA COMPRA (1 TIPO DE ARTICULO  TENIENDO EN CUENTA LAS UNIDADES COMPRADAS) MAS CARO
	 * DEL USUARIO CUYO NOMBRE EL ES DADO POR P�RAMETRO
	 * @param nombre --> EL NOMBRE DEL USUARIO
	 * @return --> EL PRECIO MAS BARATO
	 */
