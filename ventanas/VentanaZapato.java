					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Zapato(p.getCodigo(), p.getColor(), p.getPrecio(), canti, p.getMarca(), p.getNombre(), p.getRutaFoto(), "Negro", true, TipoZapato.BOTAS)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Zapato(p.getCodigo(), p.getColor(), p.getPrecio(), canti, p.getMarca(), p.getNombre(), p.getRutaFoto(), "Negro",  false, TipoZapato.DEPORTIVAS)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
									num = BD.contarProductos(con);
								} catch (SQLException e2) {
					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Zapato(p.getCodigo(), p.getColor(), p.getPrecio(), canti, p.getMarca(), p.getNombre(), p.getRutaFoto(), "Rojo" ,false, TipoZapato.TACONES)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
									num = BD.contarProductos(con);
								} catch (SQLException e2) {
					...
					if(resp == JOptionPane.OK_OPTION) {
						...
						if (canti > 0) {
							...
							if (stock >= canti) {
								...
								VentanaPrincipal.tmPedidos.get(VentanaInicioSesion.n).add(new Zapato(0, p.getColor(), p.getPrecio(), canti, p.getMarca(), p.getNombre(), p.getRutaFoto(), "Marron",  false,  TipoZapato.FORMALES)); //AQUI A�ADIR EL PRODUCTO
								int num = 0;
								try {
									num = BD.contarProductos(con);
								} catch (SQLException e2) {
